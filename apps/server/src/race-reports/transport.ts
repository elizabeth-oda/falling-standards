...
export type RaceReportModelRequest = {
  items:{input:RaceReportInput;evidence:ReturnType<typeof buildRaceReportEvidence>}[];
  config:StageConfig;
  signal:AbortSignal;
};
...
export const RACE_REPORT_INSTRUCTIONS = CONTENT_POLICY_INSTRUCTIONS+'\n'+`Write incident reports for the Department of Workplace Safety in an absurd racing game.
For each creation, select one or two supplied evidence IDs and write a short headline and one dry departmental finding.
Across the batch, vary the framing and avoid repeating the same punchline.
...
Return only the requested structured output. Return exactly one item for every requested creation. Use unchanged creation IDs and evidence IDs from the corresponding creation.
...
export function openAIRaceReportTransport(apiKey:string,fetchImpl?:typeof fetch):RaceReportTransport {
  ...
  return {async run(request) {
    const response = await client.responses.create({
      ...
      input:JSON.stringify({items:request.items.map(({input,evidence})=>({creationId:input.creationId,creationName:input.displayName,evidence}))}),
