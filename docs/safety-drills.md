...
Every shared race effect uses one persistent HUD card with a fixed family or recipe title, instruction, and layout, plus a whole-second countdown. Local changes update a short status in reserved space; amber preparation, red hazards, and green benefits or protection color that status instead of the whole card. Charge commitments and Inspection's release/steer cues update immediately. Rapids keeps its route advice while the status identifies flow, fast current, or eddy; Reconstruction keeps one instruction about forming and solid copies; Orbit keeps its outward-steering advice. Feedback reads existing spatial snapshots, and cumulative totals remain labeled as history rather than current contact. Finished racers retain the shared event summary without steering instructions.

Buddy keeps its neutral **BUDDY TETHER** card through warning, pulling, and slack. Its guidance stays fixed; only the always-visible pull meter and whole-second countdown update. With no attached tether, the same card says **No tether attached. Keep racing.** and keeps an empty meter.

Inspection keeps its rule and card shape fixed while the reserved status and configured steering-key hint distinguish releasing steering, steering freely, and protected or penalized states. A ring still marks the player's actual warning/watching coverage. The movement-exposure meter always reserves space, showing the runtime's grace accumulator only while watched or moving; warning, clear, resting, protected, and penalty-cooldown states explicitly mark it inactive. A delivered or blocked penalty remains explained for its existing cooldown. Cone rims and boundary rails reinforce the amber/red/blue phases. Release steering in red light and let residual sideways drift settle; change lanes in blue light or outside the cones. HUD simplification changes presentation only: no extra reaction time, clock, movement rule, cone bounds, forces, or rival behavior.
