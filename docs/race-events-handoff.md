...
- Main-race Live AI allows up to two fresh attempts (up to six API calls), each with a unique attempt UUID created automatically when recording starts. Server admission and allowance remain per attempt; lab requests create the same compatibility metadata on deliberate submission. Voice and generation have no separate payment opt-in checkbox. The optional post-race report uses its own setting and adds at most one paid generation call.
