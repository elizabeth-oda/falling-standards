...
The active-effect HUD uses one stable card per effect: a fixed title and instruction, whole-second countdown, and reserved local-status area. Status text and its color can change as coverage changes; the card's layout and guidance stay fixed. Debris always gives the same lane-change instruction, with a small overhead-rock warning. Vortex and Slipstream report current field coverage in the status, and Shockwave keeps its recovery advice. V4 drills use the same shell, with immediate charge and Inspection control cues and reserved meter space where needed; see [drill feedback](safety-drills.md). These are read-only presentation changes, with no gameplay or runtime changes.
...
- Main-race Live AI voice creation allows up to two fresh attempts (up to six paid API calls), each with a unique attempt UUID created automatically when recording starts. Server admission and allowance remain per attempt; lab requests create the same compatibility metadata on deliberate submission. Voice and generation have no separate payment opt-in checkbox. The optional incident-report setting permits up to two additional paid calls, for a combined ceiling of eight.
